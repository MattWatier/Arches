...
gulp.task("construct", function () {
	...
	var noframe_acc = base.pipe(clone()).pipe(rename("noframework_acc.scss")).pipe(header(fs.readFileSync(PATHS.SCSS + "/recipes/__recipes.noframe.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.uc_only.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.base.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/styleguide/_color-codes.scss", "utf8"), {
		pkg: pkg
		...
		pkg: pkg
	})).pipe(header("/** Utility Class with No Framework Shims **/", {
		pkg: pkg
	}));
	var uconly = base.pipe(clone()).pipe(rename("uconly_acc.scss")).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.uc_only.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.base.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/styleguide/_color-codes.scss", "utf8"), {
	...
	var boot_acc = base.pipe(clone()).pipe(rename("boot_acc.scss")).pipe(header(fs.readFileSync(PATHS.SCSS + "/recipes/__recipes.boot.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.boot.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.base.scss", "utf8"), {
		pkg: pkg
	...
	var boot_cvquality = base.pipe(clone()).pipe(rename("boot_cvquality.scss"))
		.pipe(header(fs.readFileSync(PATHS.SCSS + "/recipes/__recipes.cvquality.scss", "utf8"), {
			pkg: pkg
		})).pipe(header(fs.readFileSync(PATHS.SCSS + "/recipes/__recipes.boot.scss", "utf8"), {
			pkg: pkg
		})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.boot.scss", "utf8"), {
			pkg: pkg
		})).pipe(header(fs.readFileSync(PATHS.SCSS + "/components/__components.base.scss", "utf8"), {
		...
		.pipe(header(fs.readFileSync(PATHS.SCSS + "/gulp_header/__setup.boot.scss", "utf8"), {
			...
			pkg: pkg
		})).pipe(header(fs.readFileSync(PATHS.SCSS + "/gulp_header/__brand.cvquality.scss", "utf8"), {
			pkg: pkg
		})).pipe(header("/** Utility Class Only With Basic ACC Branding **/", {
			pkg: pkg
		}));

	return merge(uconly, zurb_acc, boot_acc, noframe_acc, boot_cvquality).pipe(header(fs.readFileSync(PATHS.SCSS + "/gulp_header/__preheader.scss", "utf8"), {
		pkg: pkg
	})).pipe(header(banner, {
		pkg: pkg
...
gulp.task("styleguide", gulp.series(run("npm run index && npm run uc && npm run zurb_acc &&  npm run boot_acc &&  npm run boot_cvquality")));
